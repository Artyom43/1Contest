...
int main() {
    ...
    AdjListGraph g(vertex_count + 1, false);
    std::vector<Edge> edges;
    DSU vertices(vertex_count + 1);
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        size_t  weight;
        std::cin >> weight;
        g.addEdge(0, i + 1, weight);
        edges.emplace_back(0, i + 1, weight);
    }
    std::sort(edges.begin(), edges.end(), Cmp());
    size_t cost_of_saving_world = GraphProcessing::KruskalReturnsWeight(g, edges, vertices);
    std::cout << cost_of_saving_world;
    return 0;
}
